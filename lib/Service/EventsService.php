...
class EventsService {
	...
	public function harmonize(string $uid, ServiceEntity $service, RemoteClient $remoteStore) {
		...
		$this->localEventsService = $this->localFactory->eventsService();
	...
	public function harmonizeCollection(CollectionEntity $collection): HarmonizationStatistics {
		...
		try {
			$remoteEntityDelta = $this->remoteEventsService->entityDelta($remoteCollectionId, $remoteCollectionSignature);
